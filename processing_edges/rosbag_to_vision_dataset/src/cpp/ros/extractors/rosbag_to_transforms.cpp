...
void processCameraInfo(const std::vector<sensor_msgs::CameraInfo>& infos, const std::string& output_path) {
    ...
    cam_json["D"] = cam.D;
    cam_json["K"] = {cam.K[0], cam.K[1], cam.K[2], cam.K[3], cam.K[4], cam.K[5], cam.K[6], cam.K[7], cam.K[8]};
    cam_json["R"] = {cam.R[0], cam.R[1], cam.R[2], cam.R[3], cam.R[4], cam.R[5], cam.R[6], cam.R[7], cam.R[8]};
    cam_json["P"] = {cam.P[0], cam.P[1], cam.P[2], cam.P[3], cam.P[4], cam.P[5], cam.P[6], cam.P[7], cam.P[8], cam.P[9], cam.P[10], cam.P[11]};
...
int main(int argc, char** argv) {
    ...
    if (argc != 8) {
        std::cerr << "Usage:\n  rosbag_to_json <input.bag> <dynamic_output.json> <static_output.json> <camera_info.json> <tf_topic> <tf_static_topic> <camera_info_topic>\n";
        return 1;
    }
    ...
    std::string tf_topic = argv[5];
    std::string tf_static_topic = argv[6];
    std::string camera_info_topic = argv[7];
    ...
    for (const rosbag::MessageInstance& m : view) {
        ...
        } else if (m.getTopic() == tf_static_topic) {
            auto tf_msg = m.instantiate<tf2_msgs::TFMessage>();
            if (tf_msg) tf_static_msgs.push_back(*tf_msg);
        } else if (m.getTopic() == camera_info_topic) {

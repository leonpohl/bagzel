...
def _rosbag_to_transforms_impl(ctx):
    ...
    # Topic names
    tf_topic = ctx.attr.tf_topic
    tf_static_topic = ctx.attr.tf_static_topic
    camera_info_topic = ctx.attr.camera_info_topic
...
rosbag_to_transforms = rule(
    ...
    attrs = {
        ...
        "tf_topic": attr.string(mandatory = True),
        "tf_static_topic": attr.string(mandatory = True),
        "camera_info_topic": attr.string(mandatory = True),
    },
)
